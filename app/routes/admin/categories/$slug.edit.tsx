...
import { json, redirect, useCatch, useLoaderData, useTransition } from "remix";
...
import { getCategoryBySlug, updateCategory } from "~/models/category";
...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  const updatedCategory = await updateCategory(name, slug, category.id);
  if (updatedCategory) {
    return redirect(`/admin/categories/${updatedCategory.slug}/edit`);
  } else {
      ...
      >
        ...
        <div className="flex justify-between items-center gap-6 flex-col sm:flex-row-reverse mt-6">
          ...
          <Button type="submit" role="primary" className="w-full sm:w-auto">
            {transition.state === "submitting" ? "Updating..." : "Save"}
          </Button>
          <ButtonLink
            role="default"
            to="/admin/categories"
...
export function ErrorBoundary({ error }: { error: Error }) {
  const errorMsg = error.message.replace("Invariant failed: ", "");
  return (
    <Card className="flex-col gap-7 w-full h-full text-center">
      ...
      <h1 className="text-3xl text-gray-700 font-bold">{errorMsg}</h1>
      <p className="text-gray-500">
        There was an error, finding the category !
      </p>
      <BasicLink to="/admin/categories" role="primary">
